...
function ProductPage() {
    ...
    return (
        <section className={cn(styles.wrapper, 'container')}>
            <p className={styles.title} tabIndex={0}>Product 5</p>
            <section className={styles.product}>
                <section className={styles.slider}>
                    ...
                    <div className={styles.items}>
                        {
                            imgArr.map((item, i) => (
                                <img key={`${item}-${i}`} src={item} alt='Puma Force 1 Shadow' className={cn(styles.img, {[styles.activeImg]: i === activeImg})} onClick={() => changeMainImg(i)}/>
                            ))
                        }
                    </div>
                </section>
